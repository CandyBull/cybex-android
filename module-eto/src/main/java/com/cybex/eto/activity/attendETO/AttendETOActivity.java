...
//                        mEtoProject.getBase_accuracy()), Double.parseDouble(amountStr)));
        ...
        mQuantityEt.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (message.length() == 0) {
                    ...
                    String value = mIsUseBaseToken ? message : String.valueOf(new BigDecimal(message).multiply(new BigDecimal(String.valueOf(mRate))));
    ...
    private void showDataFromEtoProject(EtoProject etoProject) {
        ...
        BigDecimal totalRemainedWhenUseBaseToken =  new BigDecimal(String.valueOf(etoProject.getCurrent_remain_quota_count())).multiply(new BigDecimal(String.valueOf(mRate)));
        mEtoTotalRemainingTv.setText(String.format(Locale.US, "%s %s", mIsUseBaseToken ? String.valueOf(totalRemainedWhenUseBaseToken.setScale(6, RoundingMode.HALF_UP).doubleValue()): String.valueOf(etoProject.getCurrent_remain_quota_count()), AssetUtil.parseSymbol(mEtoProject.getUser_buy_token()) ));
    ...
    private void showTransferConfirmationDialog() {
        CybexDialog.showAttendETOConfirmDialog(this, getResources().getString(R.string.attend_eto_dialog_title), String.format(getResources().getString(R.string.attend_eto_dialog_message), mEtoProject.getName()),
                String.format(Locale.US,"%.4f %s", mIsUseBaseToken ? Float.parseFloat(mQuantityEt.getText().toString().trim()) : Float.parseFloat(mValue),
    ...
    private void checkQuantityEtStatus(String amountStr) {
        ...
        } else if (amount.doubleValue() < (mIsUseBaseToken ? ((double) mEtoProject.getBase_min_quote()) : AssetUtil.divide(mEtoProject.getBase_min_quote(),mRate))){
            mIsBalanceEnough = false;
            mErrorLinearLayout.setVisibility(View.VISIBLE);
